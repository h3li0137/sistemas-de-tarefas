...
def add_all_tasks():
    for task, category in [("Tarefa 1", "Trabalho"), ("Tarefa 2", "Pessoal"), ("Tarefa 3", "Estudos")]:
        tasks.append((task, category))
        listbox.insert(tk.END, f"{task} [{category}]")
    save_tasks()
    update_status()
...
# Lista de tarefas
listbox = tk.Listbox(root, width=60, height=10, font=("Segoe UI", 10))
...
ttk.Button(button_frame, text="📥 Adicionar Todas", command=add_all_tasks).grid(row=1, column=0, padx=5, pady=5)
